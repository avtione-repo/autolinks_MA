...
% \date{01.01.2018} % falls ein bestimmter Tag eingesetzt werden soll, einfach
...
% Bei einem Autor werden meist die ersten drei bis vier Buchstaben des Nachnamens verwendet (Beispiel: \cite{Beut2009}), bei zwei Autoren werden die ersten zwei Anfangsbuchstaben des Nachnamens verwendet (Beispiel: \cite{InBr2009}), bei drei Autoren die ersten zwei Anfangsbuchstaben des Nachnamens des ersten Autors gefolgt von den Anfangsbuchstaben der weiteren Autoren (Beispiel: \cite{ThKZ2002}). Bei vier oder mehr Autoren werden die Anfangsbuchstaben der Nachnamen der ersten vier Autoren verwendet (Beispiel: \cite{HSFN2009}).
...
\usepackage[utf8]{inputenc} % Erlaub die direkte Eingabe spezieller Zeichen.
                            %  Utf8 muss die Eingabekodierung des Editors sein.
\usepackage{microtype}      % Optimale Randausrichtung und Skalierung.           % Korrekte Anfuehrungszeichen in der Literaturliste.
\usepackage[
  newcommands
]{ragged2e}                 % Verbesserte \ragged...Befehle
\PassOptionsToPackage{
...
% }}}
             ...
             % Biblatex mit alphabetischem Style und biber.
\bibliography{\jobname.bib}      % Dateiname der bib-Datei.
...
\usepackage[
    printonlyused
    ]{acronym}              % Abkuerzungsverzeichnis.           %  bereits integriert.
%\usepackage{breakurl}
%\def\UrlBreaks{\do\/\do-}
\usepackage{listings}       % Spezielle Umgebung für...
    \lstset{                %  ...Quelltextformatierung.
